...
See PATTERNS in ssh_config(5) for more information on patterns. This keyword may appear multiple times in sshd_config with each instance appending to the list.`,
 ...
 The default value is 3. If ClientAliveInterval is set to 15, and ClientAliveCountMax is left at the default, unresponsive SSH clients will be disconnected after approximately 45 seconds. Setting a zero ClientAliveCountMax disables connection termination.`,
	...
	"ExposeAuthInfo": common.NewOption(
	...
	"GSSAPIAuthentication": common.NewOption(
		`Specifies whether user authentication based on GSSAPI is allowed. The default is no.`,
		BooleanEnumValue,
	),
	"GSSAPICleanupCredentials": common.NewOption(
		`Specifies whether to automatically destroy the user's credentials cache on logout. The default is yes.`,
		BooleanEnumValue,
	),
	"GSSAPIStrictAcceptorCheck": common.NewOption(
	...
	"HostbasedUsesNameFromPacketOnly": common.NewOption(
 ...
 Accepted values are yes (the default) to ignore all per- user files, shosts-only to allow the use of .shosts but to ignore .rhosts or no to allow both .shosts and rhosts.`,
	...
	"KerberosAuthentication": common.NewOption(`Specifies whether the password provided by the user for PasswordAuthentication will be validated through the Kerberos KDC. To use this option, the server needs a Kerberos servtab which allows the verification of the KDC's identity. The default is no.`,
		BooleanEnumValue,
	),
	"KerberosGetAFSToken": common.NewOption(`If AFS is active and the user has a Kerberos 5 TGT, attempt to acquire an AFS token before accessing the user's home directory. The default is no.`,
		BooleanEnumValue,
	),
	"KerberosOrLocalPasswd": common.NewOption(`If password authentication through Kerberos fails then the password will be validated via any additional local mechanism such as /etc/passwd. The default is yes.`,
		BooleanEnumValue,
	),
 ...
 Independent of this setting, the permissions of the selected tun(4) device must allow access to the user.`,
	...
	"PerSourceMaxStartups": common.NewOption(`Specifies the number of unauthenticated connections allowed from a given source address, or “none” if there is no limit. This limit is applied in addition to MaxStartups, whichever is lower. The default is none.`,
		docvalues.OrValue{
			Values: []docvalues.Value{
				docvalues.EnumValue{
					EnforceValues: true,
 ...
 Note that disabling X11 forwarding does not prevent users from forwarding X11 traffic, as users can always install their own forwarders.`,
	...
	"XAuthLocation": common.NewOption(`Specifies the full pathname of the xauth(1) program, or none to not use one. The default is /usr/X11R6/bin/xauth.`,
		docvalues.StringValue{},
	),
}
