...
var UmsdosDocumentationAssignable = map[docvalues.EnumString]docvalues.Value{
	...
	docvalues.CreateEnumStringWithDoc(
		"nfs",
		"Enable this only if you want to export the FAT filesystem over NFS. To maintain backward compatibility, -o nfs is also accepted, defaulting to stale_rw",
	): docvalues.EnumValue{
		EnforceValues: true,
		Values: []docvalues.EnumString{
			docvalues.CreateEnumStringWithDoc(
...
var UmsdosDocumentationEnums = []docvalues.EnumString{
	docvalues.CreateEnumStringWithDoc(
		"debug",
