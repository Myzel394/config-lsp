...
var Options = map[string]docvalues.DocumentationValue{
	...
	"bind": {
		...
		Value: docvalues.SuffixValue{
			Suffixes: []docvalues.Suffix{
				{
					Suffix:  "=onion",
					Meaning: "Tag incoming connections to this address and port as Tor onion connections.",
				},
			},
			Required: false,
			SubValue: docvalues.IPAddressValue{
				AllowIPv4: true,
				AllowIPv6: true,
				AllowPort: docvalues.AllowedStatusRequired,
			},
		},
	},
	"cjdnsreachable": {
	...
	"onion": {
		...
		Value: docvalues.OrValue{
			Values: []docvalues.DeprecatedValue{
				...
				docvalues.PrefixValue{
					Prefixes: []docvalues.Prefix{
						{
							Prefix:  "unix:",
							Meaning: "Local file path",
						},
					},
	...
	"torcontrol": {
		...
		Value: docvalues.IPAddressValue{
			AllowIPv4: true,
			AllowIPv6: true,
			AllowPort: docvalues.AllowedStatusAllowed,
		},
	},
	"torpassword": {
	...
	"whitebind": {
		...
		Value: docvalues.KeyValueAssignmentValue{
			Key: docvalues.EnumValue{
				EnforceValues: true,
				Values: []docvalues.EnumString{
					docvalues.CreateEnumStringWithDoc("bloomfilter", "Allow requesting BIP37 filtered blocks and transactions"),
					docvalues.CreateEnumStringWithDoc("noban", "Do not ban for misbehavior; implies download"),
					docvalues.CreateEnumStringWithDoc("forcerelay", "Relay transactions that are already in the mempool; implies relay"),
					docvalues.CreateEnumStringWithDoc("relay", "Relay even in -blocksonly mode, and unlimited transaction announcements"),
					docvalues.CreateEnumStringWithDoc("mempool", "Allow requesting BIP35 mempool contents"),
					docvalues.CreateEnumStringWithDoc("download", "Allow getheaders during IBD, no disconnect after maxuploadtarget limit"),
					docvalues.CreateEnumStringWithDoc("addr", "Responses to GETADDR avoid hitting the cache and contain random records with the most up-to-date info"),
				},
			},
			Value: docvalues.IPAddressValue{
				AllowIPv4: true,
	...
	"whitelist": {
		...
		Value: docvalues.PrefixValue{
			Prefixes: []docvalues.Prefix{
				{Prefix: "bloomfilter", Meaning: "Allow requesting BIP37 filtered blocks and transactions"},
				{Prefix: "noban", Meaning: "Do not ban for misbehavior; implies download"},
				{Prefix: "forcerelay", Meaning: "Relay transactions that are already in the mempool; implies relay"},
				{Prefix: "relay", Meaning: "Relay even in -blocksonly mode, and unlimited transaction announcements"},
				{Prefix: "mempool", Meaning: "Allow requesting BIP35 mempool contents"},
				{Prefix: "download", Meaning: "Allow getheaders during IBD, no disconnect after maxuploadtarget limit"},
				{Prefix: "addr", Meaning: "Responses to GETADDR avoid hitting the cache and contain random records with the most up-to-date info"},
				{Prefix: "in", Meaning: "Apply permissions to incoming connections"},
				{Prefix: "out", Meaning: "Apply permissions to outgoing connections"},
			},
			SubValue: docvalues.IPAddressValue{
				AllowIPv4:  true,
				AllowIPv6:  true,
				AllowRange: docvalues.AllowedStatusAllowed,
			},
		},
	},
	"addresstype": {
	...
	"debug": {
		...
		Value: docvalues.EnumValue{
			Values: []docvalues.EnumString{
				...
				docvalues.CreateEnumString("addrman"),
				docvalues.CreateEnumString("bench"),
				docvalues.CreateEnumString("blockstorage"),
				docvalues.CreateEnumString("cmpctblock"),
				docvalues.CreateEnumString("coindb"),
				docvalues.CreateEnumString("estimatefee"),
				docvalues.CreateEnumString("http"),
				docvalues.CreateEnumString("i2p"),
				docvalues.CreateEnumString("ipc"),
				docvalues.CreateEnumString("leveldb"),
				docvalues.CreateEnumString("libevent"),
				docvalues.CreateEnumString("mempool"),
				docvalues.CreateEnumString("mempoolrej"),
				docvalues.CreateEnumString("net"),
				docvalues.CreateEnumString("proxy"),
				docvalues.CreateEnumString("prune"),
				docvalues.CreateEnumString("qt"),
				docvalues.CreateEnumString("rand"),
				docvalues.CreateEnumString("reindex"),
				docvalues.CreateEnumString("rpc"),
				docvalues.CreateEnumString("scan"),
				docvalues.CreateEnumString("selectcoins"),
				docvalues.CreateEnumString("tor"),
				docvalues.CreateEnumString("txpackages"),
				docvalues.CreateEnumString("txreconciliation"),
				docvalues.CreateEnumString("validation"),
				docvalues.CreateEnumString("walletdb"),
				docvalues.CreateEnumString("zmq"),
			},
			EnforceValues: false,
		},
	},
	...
	"rpcallowip": {
		Documentation: "Allow JSON-RPC connections from specified source. Valid values for <ip> are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0), a network/CIDR (e.g. 1.2.3.4/24), all ipv4 (0.0.0.0/0), or all ipv6 (::/0). This option can be specified multiple times",
		Value: docvalues.IPAddressValue{
			AllowIPv4:  true,
			AllowIPv6:  true,
			AllowRange: docvalues.AllowedStatusAllowed,
